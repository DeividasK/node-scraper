...
const { downloadFloorPlan, downloadImages } = require('./getImages');
...
async function getListing(url) {
  ...
  try {
    ...
    await Promise.all([
      downloadImages(applicationJsonScriptTag, {
        directory: LISTINGS_DIRECTORY,
        listingId
      }),

      downloadFloorPlan(html, {
        directory: LISTINGS_DIRECTORY,
        listingId
      })
    ]);
    ...
    console.log(`Listing ${listingId} crashed.`);
    console.log(error);
    return {};
  }
}
